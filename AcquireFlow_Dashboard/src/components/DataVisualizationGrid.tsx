...
export const DataVisualizationGrid = ({
  ...
  const renderChartHeader = (title, description) => <div className="flex justify-between items-start mb-4">
      <div className="flex items-center">
        <h3 className="font-medium text-dark">{title}</h3>
        {description && <div className="relative group ml-2.5">
            <Info size={14} className="text-gray-400" />
            <div className="absolute left-0 bottom-full mb-2 w-64 bg-white p-3 text-xs text-gray-600 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
              {description}
            </div>
          </div>}
      </div>
      <div className="flex space-x-2">
        <button className="p-1.5 text-gray-400 hover:text-gray-600 transition-colors" onClick={() => handleRefresh(title)}>
          <RefreshCcw size={14} />
        </button>
        <button className="p-1.5 text-gray-400 hover:text-gray-600 transition-colors" onClick={() => handleDownload(title)}>
          <Download size={14} />
        </button>
      </div>
    </div>;
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {/* Market Trends Chart */}
      <div className="bg-white rounded-xl p-5 border border-gray-200 shadow-sm col-span-1 lg:col-span-2">
      ...
      <div className="bg-gradient-to-br from-primary to-primary-dark rounded-xl p-6 border border-primary shadow-sm text-white col-span-1 lg:col-span-3 flex flex-col md:flex-row justify-between items-center">
        <div>
          <h3 className="font-bold text-xl mb-3">
            Investment Opportunity Alert
          </h3>
          <p className="text-white text-opacity-90 mb-5">
        ...
        <div className="mt-5 md:mt-0">
          <button className="bg-white text-primary px-6 py-3 rounded-xl font-medium flex items-center hover:shadow-lg transition-all">
            View Opportunities
